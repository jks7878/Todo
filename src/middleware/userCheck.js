...
module.exports = {
    checkUserInfo: async function (req, res, next) {
        try {
            ...
            const [exist] = await todoUser.getTodoUser({ USER_ID: req.body.USER_ID });
            if(exist.length > 0) {
    ...
    checkUserExist: async function (req, res, next) {
        try {
            req.cond = checkParam(req.params.param || req.params.seq || req.body.REG_USER_SQ);

            const [userCntRes] = await todoUser.getTodoUserCnt(req.cond);

            if(!userCntRes || userCntRes[0].CNT == 0) throw new CustomError(404, "Cannot Find User");
